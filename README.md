...
### 1. 单机模式（默认）
- **数据库**：使用本地 `sqlite:///./transcode.db`，开箱即用。
- **存储**：直接读取和写入本地目录（如 `./data/in` 和 `./data/out`）。
- **适用场景**：个人测试、单台高性能服务器上的多进程并发转码。

### 2. 分布式模式（多节点集群）
- **数据库**：使用集中式的 PostgreSQL（需通过 `DB_URL` 覆盖，如 `postgresql+psycopg://user:pass@host:5432/db`）。
- **共享存储**：所有的计算节点（Master 和 Worker）必须通过网络共享协议（如 **NFS、SMB** 或对象存储网关）挂载到**完全一致的本地路径**。
  - 例如：所有机器都执行 `mount -t nfs 10.0.0.1:/data /app/data`。
- **适用场景**：多台物理机/虚拟机协同转码，横向扩展吞吐量。
